...
- **Hard permission boundary**: the WeChat scenario is unattended, so the permission layer confines every write to the session's own isolated workspace and rejects destructive/irreversible commands and interactive tools outright; reading files, searching and web access are always allowed. Rejection reasons are returned to the AI, which typically retries along a safe path without interrupting the conversation. Full rules in the [Agent Permission Model](docs/permissions.md) (Chinese).
