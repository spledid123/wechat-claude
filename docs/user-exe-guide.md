...
- Windows 10/11 x64。
- 一个可用的微信机器人登录二维码。
- 可访问微信 iLink Bot API 与模型 API 的网络（首次运行还需要联网下载 Claude 运行时，约 218MB）。
- 在 exe 旁放好 `.env`（API Key / 端点，模板见 `.env.example`）。
...
exe 包内已内置：Tauri 壳、Node 22 运行时、项目编译后的 JavaScript 与全部 JS 依赖。**Claude 运行时（`claude.exe`，约 218MB）不随包分发**——首次运行时安装向导会自动下载到数据目录 `.wechat-claude
untime\claude\`，之后常驻本机、离线可用。下载源为 npmmirror（备源 npmjs），与模型 API 一样只需普通网络。

Python 不是基础运行依赖。普通聊天、微信收发、定时任务、自动发送已有文件、**图片理解（vision）**都不需要 Python。安装向导里的"Python 文档解析环境"是可选项（约 360MB，含 uv 自动托管的 CPython + markitdown + pymupdf），装与不装都不影响其余功能。

## 首次运行：安装向导

第一次双击 exe 时会弹出"首次运行安装向导"窗口，两个卡片：

| 步骤 | 内容 | 说明 |
| --- | --- | --- |
| ① Claude Agent 运行时（必需） | 下载 `claude.exe`（约 218MB），带进度显示 | 装完自动继续启动，随后自动打开管理面板 |
| ② Python 文档解析环境（可选） | 下载 uv 并创建 `.venv`（约 360MB） | 不装不影响聊天与图片；装完即有 PDF/Office 解析 |

下载失败可点"重试"。**Claude 运行时装完后窗口不会自动关闭、也不会自动弹浏览器**——你可以接着点 Python 安装，或点页面里的"打开管理面板"，或自己关掉窗口；关闭只是隐藏，随时从托盘菜单或面板头部的**"组件安装"**入口回来（进入面板后从托盘再点"组件安装"会自动回到安装页；服务运行中也能补装 Python，装完即生效、无需重启）。之后再启动 exe 若运行时已就绪则直接进托盘；向导页固定地址 `http://127.0.0.1:8787/install`，浏览器也可直接访问。

## 添加自己的技能（skills/，开放扩展）

`skills/` 文件夹是开放的：往里面放一个新文件夹（内含 `SKILL.md`），AI 就会把它当作可用的参考资料。无需改代码、无需重启。

```text
skills/
└── 我的知识库/            ← 你新建的文件夹，名字随意（建议英文，避免路径兼容问题）
    ├── SKILL.md           ← 必需；frontmatter 写一行 description（AI 靠它判断何时用）
    └── （其他任意参考文件）
```
...
```markdown
---
description: 一句话说明这个技能做什么、什么时候用（会进入 AI 的系统提示词）
---
具体的参考资料、操作说明、示例……
```
...
| 操作 | 生效时机 |
| --- | --- |
| 新增技能文件夹 | **下一条消息**自动同步进会话工作区并出现在提示词里 |
| 查看/确认已识别 | 管理面板“设置”页“参考技能”卡片列出全部技能 |
| 修改已有技能内容 | 需在微信发 `/new` 重建会话（已复制的旧内容不自动刷新） |
| 删除技能 | 从 skills/ 移除文件夹；旧会话仍留副本，同样 `/new` 清理 |

注意：每个会话工作区各复制一份 skills（现状约 1MB × 会话数），别放超大文件；`description` 建议控制在一两句话，它会计入每次请求的提示词。
...
双击 exe 后，程序会常驻系统托盘（首次运行先见上文"安装向导"）。左键单击托盘图标直接打开管理面板；右键菜单包含：

- 状态行（当前状态：启动中 / 待扫码登录 / 运行中 / 首次运行安装 等）。
- 打开管理面板。
- 组件安装（重新打开安装向导，可补装 Python 环境）。
- 打开数据目录。
- 打开日志文件。
- 重启服务。
- 退出。

如果双击后没有看到窗口，这是正常的；请看 Windows 右下角托盘图标。退出请走托盘菜单"退出"，它会先让服务优雅收尾再结束进程；直接结束 exe 进程也不会留下后台残留。
...
- 资源占用卡：服务进程与 Claude 运行时的 CPU/内存、系统内存/CPU（每 5 秒后台采样，面板只读缓存，自身开销可忽略）。
...
- **系统提示词**：统一流程——「插入默认模板」/「导入提示词文件」（选本地 md 读入编辑器）/「恢复官方预设」→ 在编辑器里修改 → 保存并应用（两击确认，留空保存即回官方预设）。内容保存到数据目录 `system-prompt.md`，每条消息时从磁盘现读注入（修改对下一条消息立即生效）。预览区展示实际注入构成：官方预设快照（内嵌于 claude.exe 无法直接读取，点「捕获官方提示词」可离线获取逐字全文，不联网、不耗 API；升级 claude.exe 后需重新捕获）+ 微信功能块（替换模式下依然保留）。内置记忆系统已全局关闭（省约 2.7k token/轮，也避免读到其他项目的记忆文件）。替换模式下官方预设完全不注入：自定义身份/人设，适用于聊天、角色扮演、写作等场景，每轮再省约 3k 输入 token（模型家族说明、Claude Code 推广等段落一并消失）；工具与权限硬边界不受影响。
- 查看和删除按发送者拆分的原始报文记录。
...
数据目录固定在 exe 所在目录旁的 `.wechat-claude\`（想放别处可设 `WECHAT_CLAUDE_DATA_DIR` 环境变量）。旧版 Electron exe 曾把数据写到 `Temp` 目录，若你从旧版迁移：把旧 Temp 目录里的 `.wechat-claude` 整个复制到新 exe 旁边，再启动即可。
...
确认程序正在运行。管理面板固定地址为 http://127.0.0.1:8787/（可用 WECHAT_ADMIN_PORT 改端口），托盘状态行也会显示当前状态。
