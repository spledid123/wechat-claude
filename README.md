...
- **系统提示词自定义**：管理面板可预览当前实际注入的提示词构成——官方 Claude Code 预设（内嵌于 claude.exe，一键离线捕获逐字全文；内置记忆系统已关闭，预设实测约 3.2k token）+ 微信功能块；也可将官方预设整体替换为任意 md 文件，自定义身份/人设，适用于聊天、角色扮演、写作等场景，实测每轮再省约 3k 输入 token（模型家族/Claude Code 推广等段落一并消失）。微信功能块与权限硬边界不受影响。
- **本地管理面板**：浏览器里配置模式/模型/API 接入与视觉通道、自定义系统提示词、管理历史会话、查看按发送者拆分的报文记录与 Agent 处理流程实时事件流。
